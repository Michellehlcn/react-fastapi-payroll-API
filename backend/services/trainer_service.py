...
class TrainerService:
    ...
    @staticmethod
    def read_trainer_byID(trainer_id:int, user: user_schema.User,db:_orm.Session):
        """return a trainer that matches trainer_id"""
        record = db.query(Trainer).filter_by(owner_id=user.id).filter(Trainer.id == trainer_id).first()
        if not record:
    ...
    @staticmethod
    def read_trainer(trainer_id:int, user: user_schema.User, db:_orm.Session):
        record = read_trainer_byID(trainer_id=trainer_id, user= user, db=db)
        return trainer_schema.TrainerOut.from_orm(record)

    @staticmethod
    def update_trainer_record(trainer_id:int, data:CreateTrainer, user: user_schema.User, db:_orm.Session):
        """update trainer record"""
        record = read_trainer_byID(traienr_id, user, db)
    ...
    @staticmethod
    def deactivate_trainer(trainer_id:int, user: user_schema.User, db:_orm.Session):
