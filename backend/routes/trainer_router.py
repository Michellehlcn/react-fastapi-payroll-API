...
async def trainer_by_id(trainer_id:int,db:_orm.Session = Depends(_database.get_db), user: user_schema.User = Depends(user_router.get_current_user) ):
    return trainer_service.TrainerService.read_trainer(trainer_id=trainer_id, user=user, db=db)
